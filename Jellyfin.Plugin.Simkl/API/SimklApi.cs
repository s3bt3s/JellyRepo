...
using Jellyfin.Plugin.SimklWatched.API.Exceptions;
using Jellyfin.Plugin.SimklWatched.API.Objects;
using Jellyfin.Plugin.SimklWatched.API.Responses;
using MediaBrowser.Common.Net;
using MediaBrowser.Model.Dto;
using Microsoft.Extensions.Logging;

namespace Jellyfin.Plugin.SimklWatched.API
{
    /// <summary>
    /// Simkl Api.
    /// </summary>
...
/// <summary>
        ...
        /// <summary>Sends one manual watched mark using film or series identifiers.</summary>
        /// <param name="item">Film or episode metadata with series IDs for episodes.</param>
        /// <param name="episode">Whether this is an episode.</param>
        /// <param name="token">User authorization.</param>
        /// <returns>Whether SIMKL confirmed the item.</returns>
        public async Task<bool> SendManualWatched(BaseItemDto item, bool episode, string token)
        {
            ...
            var result = await SyncHistoryAsync(history, token).ConfigureAwait(false);
            if (result == null || result.NotFound?.Movies.Length > 0 || result.NotFound?.Shows.Length > 0 || result.NotFound?.Episodes.Length > 0)
            {
                return false;
            }

            return episode ? result.Added.Episodes >= 1 : result.Added.Movies >= 1;
        }
        ...
        private async Task<SyncHistoryResponse?> SyncHistoryAsync(SimklHistory history, string userToken)
        {
            ...
            {
                _logger.LogError("SIMKL authorization expired; reconnect this user");
                SimklPlugin.Instance?.Configuration.DeleteUserToken(userToken);
                throw new InvalidTokenException("Invalid SIMKL authorization");
            }
        }
        ...
        private async Task<T?> Get<T>(string url, string? userToken = null)
        {
            ...
            using var responseMessage = await _httpClientFactory.CreateClient(NamedClient.Default)
                .SendAsync(options);
